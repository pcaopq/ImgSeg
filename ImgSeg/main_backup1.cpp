...
string imgname = "0281";
Mat image_ori0 = imread(imgname+".jpg", 0);
Mat image_ori;
Mat image;
Mat imageBGR;
int height;
...
template<typename T>
vector<block> regionform(vector<T>& patterns){
    long num_patterns = patterns.size();
    bool grouped;
    bool found;
    float avh, hgap, vgap;
...
void cleanoneinanother(vector<block>& one, vector<block>& another, int threshold, int areathreshold){
    //clean line in text
    vector<int> todel;
    vector<int> merge;
    ...
    for(int i = 0;i < another.size();i++){
        ...
        if(todel.size() >= 2){ //6.28: todel.size() >= 2 only change the txt blocks
...
bool check_tt_near(block A, block B){
...
void merge_blocks(vector<block>& blocks, int w, int h, int area_threshold, vector<block> oblk){
    //near && check_tt_near
    long bs = 0;
    vector<int> todel;
...
void formetablk(vector<block>& titleblock, vector<block>& txtblock){
    long bs = 0;
    long ps = 0;
    vector<block> lines;
...
//    for(int i = 0;i<10;i++)
    {
        ...
        merge_blocks(txtblock, 0, 10, txta/2.0, titleblock);
        merge_blocks(titleblock, 10, 0, titlea/2.0, txtblock);
        
        // bounder threshold and area threshold
        // near or included in txt
        // delete txt < 10*txta
        txta = get_min_area(txtblock);
        titlea = get_min_area(titleblock);
        cleanoneinanother(txtblock, titleblock, 10, 10*txta);
        //delete title < 10*titlea
        cleanoneinanother(titleblock, txtblock, 10, 10*titlea);
    }
}
...
//Binarize the image
    ...
    threshold(image_ori, image, 5, 255.0, THRESH_BINARY);
...
//        count++;
//        if(Rects.size() < 60000){
//            threshold(image_ori, image, 150+count*10, 255.0, THRESH_BINARY);
//            Rects = locate_rects();
//        }else
//            break;
//    }
...
//      saveInfo(patterns, imgname+"pats.txt");
...
//    generate_boxes(text); //some textblocks could be boxes
    
//    generate_boxes(title);
//      vector<pattern> patterns;
...
//      formetablk(titleblocks, textblocks);
//    cleanregion(textblocks, 5);
//    cleanregion(titleblocks, 5);
...
//      cout<<"The real running time in total is: "<<elapsed_secs<<"s"<<endl;
      ...
      Draw(titleblocks, false, imgname+"txtblks");//false: do not print the blocks info
