...
void pattern_classfy(vector<pattern>& patterns){
...
//        cout<<blocks.size()<<endl;
        ...
        for(int j = 0;j < tmp_blk.size();j++){
            ...
            for(int k = 0;k < tmp_blk[j].patterns.size();k++){
                if(near(patterns[i], tmp_blk[j].patterns[k], hgap, vgap, 2)){
                    found = true;
                    break;
                }
            }
...
//the type of lines is pattern
    ...
    //don't use a total block parameter
...
template<typename T>
void Draw(vector<T> patterns, bool print_info){
    if(print_info){
        int count = 1;
        for(auto j:patterns){
...
void saveInfo(vector<T> patterns, string name){
    ofstream file(name);
    for(auto i:patterns)
        file<<i.left<<" "<<i.right<<" "<<i.top<<" "<<i.bottom<<" "<<i.type<<"\n";
    file.close();
}
...
//    saveInfo(new_lines, "blocks0033newlines.txt");
...
//    readInfo(patterns, "blocks0033newlines.txt");
    ...
    vector<block> blocks = regionform(patterns);
    saveInfo(blocks, "blocks0033.txt");
...
//    imwrite("Segment0033.jpg", imageBGR);
//    waitKey(0);
