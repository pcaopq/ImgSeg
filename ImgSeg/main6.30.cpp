...
void Draw(vector<T> patterns, bool print_info, string name, float factorh, float factorw){
    ...
    for(auto j:patterns){
        ...
        if(j.type != 8){
            switch(j.type){
                case 1:{
                    rectangle(imageBGR, Point(j.left*factorw, j.top*factorh), Point(j.right*factorw, j.bottom*factorh), CV_RGB(0, 0, 255), 5);
                    break;
                }
                case 2:{
                    rectangle(imageBGR, Point(j.left*factorw, j.top*factorh), Point(j.right*factorw, j.bottom*factorh), CV_RGB(0, 255, 0), 5);
                    break;
                }
                case 3:{
                ...
                case 6:{
                    rectangle(imageBGR, Point(j.left*factorw, j.top*factorh), Point(j.right*factorw, j.bottom*factorh), CV_RGB(0, 0, 0), 5);
                    break;
                }
                case 7:{
                    rectangle(imageBGR, Point(j.left*factorw, j.top*factorh), Point(j.right*factorw, j.bottom*factorh), CV_RGB(0, 0, 0), 5);
    ...
    for(int i = 0;i < num_blocks;i++){
        switch(blocks[i].type){
            case 1:{
                file<<"\t\t\t{\n\t\t\t\t"<<'"'<<"class"<<'"'<<":";
                file<<'"'<<"article"<<'"'<<','<<"\n\t\t\t\t";
                file<<'"'<<"height"<<'"'<<":"<<blocks[i].height*scaleh<<','<<"\n\t\t\t\t";
                file<<'"'<<"id"<<'"'<<":"<<i<<','<<"\n\t\t\t\t";
                file<<'"'<<"type"<<'"'<<":"<<'"'<<"rect"<<'"'<<','<<"\n\t\t\t\t";
                file<<'"'<<"width"<<'"'<<":"<<blocks[i].width*scalew<<','<<"\n\t\t\t\t";
                file<<'"'<<"x"<<'"'<<":"<<blocks[i].left*scalew<<','<<"\n\t\t\t\t";
                if(i+1 < num_blocks)
                    file<<'"'<<"y"<<'"'<<":"<<blocks[i].top*scaleh<<"\n\t\t\t},\n";
                else{
                    file<<'"'<<"y"<<'"'<<":"<<blocks[i].top*scaleh<<"\n\t\t\t}\n\t\t]\n\t\t";
                    file<<"\n\t}\n]";
                }
